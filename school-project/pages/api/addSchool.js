...
export default async function handler(req, res) {
  ...
  try {
    ...
    const newPath = path.join('/schoolImages', file.newFilename).replace(/\\/g, '/');
    ...
    const query = 'INSERT INTO schools (name, address, city, states, contact, image, email_id, board) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
    const values = [
      fields.name[0],
      fields.address[0],
      fields.city[0],
